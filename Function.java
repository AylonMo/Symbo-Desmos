...
public class Function {
    ...
    public String toString() {
        ...
        String repr = "f(x) = ", sign;
        String xExpression = "x" + ((getDegree() == 1)? (""):("^" + getDegree()));
        
        // if the coefficient is 1 or -1
        if (smartRounding(Math.abs(coefficients[getDegree()])) == 1)
            repr += xExpression;
        else
            repr += coefficients[getDegree()] + " * " + xExpression;
        ...
        for (int i = getDegree() - 1; i >= 1; i--) {
            ...
            // if the coefficient is 1 or -1
            double roundedCoeff = smartRounding(Math.abs(coefficients[i]));
            if (roundedCoeff == 1)
                repr += sign + xExpression;
            else
                repr += sign + smartRounding(Math.abs(coefficients[i])) + " * " + xExpression;
        }

        // add the constant of the function
        if (coefficients[0] != 0)
            repr += ((coefficients[0] < 0)? (" - "):(" + ")) + smartRounding(Math.abs(coefficients[0]));
        return repr;
	}
    ...
    // modified toString() for arrays using the smartRounding() method
    public String reprRoundedArray(double[] array) {
        Arrays.sort(array);
        if (array.length == 0)
            return "[]";
        ...
        String arrayRepr = "[";
        for (double x : array) {
            // skip values that appeared earlier
            if (arrayRepr.indexOf("" + x) == -1)
                arrayRepr += smartRounding(x) + ", ";
        }

        arrayRepr = arrayRepr.substring(0, arrayRepr.length() - 2) + "]";
        return arrayRepr;
    }
    ...
    /* a method to compute the division of f(x) by the factor (x - alpha) and ignoring the remainder.
    ...
    public double[] polDivRec(double[] current, double alpha, double[] resCoeffs) {
        ...
        divRes[pos - 1] = current[pos - 1] - current[pos] * alpha;  // subtract (x - a) times the leading coefficient
     ...
     * determined by the graphs of polynomial functions with an even degree, there are no roots of the function (intersection
     * points with the x axis) in the following cases:
     ...
     * otherwise, using the intermediate-value-theorem from calculus we can deduct that there is at least one root
     * of the function, because there are positive and negative values of the function.
     */
    ...
    // a method to find the root of the function, using the relevant method from above
